...
[[ $before == "$after" ]] || { printf 'FAIL: settings, saved data, indent width or active tab changed\nBefore: %s\nAfter: %s\n' "$before" "$after" >&2; exit 1; }
...
printf 'PASS: real application exception failed the runner; fixtures, lock, settings, indent width and active tab were restored.\n'
