...
(async () => {
  ...
  try {
    ...
    punctuation.value = 'period';
    punctuation.dispatchEvent(new punctuation.ownerDocument.defaultView.Event('change', {bubbles: true}));
    await test.waitFor(() => plugin.settings.listItemPunctuationPolicy === 'period', 'Grouped dropdown still updates its setting');
    const expectedSettings = {...JSON.parse(before), showGutterIcons: false, listItemPunctuationPolicy: 'period'};
