...
**By list type (GOV.UK-inspired)** applies the no-terminal-punctuation policy to bullets (`-`, `+`, `*`) and the period policy to numbered items (`1.`, `1)`). Each nested list follows its own marker type, and task items follow their underlying marker. All existing punctuation exceptions below still apply. This is a punctuation preference, not full GOV.UK validation: it does not check capitalisation, grammar, lead-ins, or whether numbered items describe a process. It does not change the separate line-ending setting or edit notes automatically.
